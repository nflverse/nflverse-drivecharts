...
ggdrive <- function(pbp,
  ...
  one_g <- pbp |>
    ...
    dplyr::mutate(
      ...
      start_abs = dplyr::case_when(
        start_desc == "KICKOFF" & end_desc_nfl == "TOUCHDOWN" & posteam == home_team & plays == 0 ~ 10L + ret_yards,
        start_desc == "KICKOFF" & end_desc_nfl == "TOUCHDOWN" & posteam == away_team & plays == 0 ~ 110L - ret_yards,
      ...
      scoring_drive = ifelse(stringr::str_detect(end_desc, "ouchdown") | stringr::str_detect(end_desc, "Field"), 1, 0),
      end_desc = dplyr::case_when(
        end_desc == "Touchdown" ~ "TD",
        end_desc == "Opp touchdown" & end_desc_nfl == "PUNT" ~ "Punt Return TD",
        end_desc == "Opp touchdown" & end_desc_nfl == "INTERCEPTION" ~ "INT Return TD",
        end_desc == "Opp touchdown" ~ "Opp TD",
        end_desc == "Field goal" ~ "FG",
        end_desc == "Missed field goal" ~ "Missed FG",
        end_desc == "Turnover" & end_desc_nfl == "INTERCEPTION" ~ "INT",
      ...
      to_location = dplyr::case_when(
        end_desc == "INT" & posteam == away_team ~ end_abs + as.integer(air_yards),
        end_desc == "INT" & posteam == home_team ~ end_abs - as.integer(air_yards),
        end_desc == "FUM" & posteam == away_team ~ end_abs + as.integer(yards_gained),
        end_desc == "FUM" & posteam == home_team ~ end_abs - as.integer(yards_gained),
        TRUE ~ NA_integer_
      ),
      label = ifelse(plays > 1,
                     glue::glue("{end_desc} / {plays} Plays {yards} Yds ({top}) / {away_score}-{home_score}"),
                     glue::glue("{end_desc} / {plays} Play {yards} Yds ({top}) / {away_score}-{home_score}")
      ),
      label_x = dplyr::case_when(
        !is.na(to_location) & posteam == away_team & end_abs <= to_location ~ to_location,
        !is.na(to_location) & posteam == home_team & end_abs >= to_location ~ to_location,
        TRUE ~ end_abs
      ),
      grob_x = dplyr::case_when(
        !is.na(to_location) ~ to_location,
        TRUE ~ end_abs
      ),
      i = abs(start_abs - end_abs) * 5,
      away_score_lab = ifelse(away_score != dplyr::lag(away_score) | drive_number == 1, away_score, ""),
      home_score_lab = ifelse(home_score != dplyr::lag(home_score) | drive_number == 1, home_score, ""),
    ) |>
    dplyr::left_join(
  ...
  half <- pbp |>
    dplyr::mutate(
      max_drive = max(fixed_drive),
      max_qtr = max(qtr)
    ) |>
    dplyr::filter(desc == "END QUARTER 2" | (desc == "END QUARTER 4" & max_qtr > 4)) |>
    dplyr::mutate(
  ...
  ko_ret_td <- one_g |>
    dplyr::filter(start_desc == "KICKOFF" & end_desc_nfl == "TOUCHDOWN" & plays == 0) |>
    dplyr::mutate(
      lab = "Kickoff\nReturn",
      lab_hjust = dplyr::case_when(
        posteam == home_team & type == "all" ~ 0,
        TRUE ~ 1
      ),
    )
    ...
    dplyr::left_join(
      pbp |>
        ...
        dplyr::mutate(
          ...
          count_color = dplyr::case_when(
            # penalty == 1 ~ "#F4B317",
            penalty == 1 ~ "#FCD32F",
            TRUE ~ "black"
          ),
          count_bg = dplyr::case_when(
            penalty == 1 ~ "grey30",
            TRUE ~ NA_character_
          ),
        ) |>
        ...
        dplyr::ungroup() |>
        dplyr::select(drive_number, yard_abs, play_count, penalty, count_color, count_bg),
      by = "drive_number"
    )

  if (type == "home" && isTRUE(home_reverse)){
    rev_vars <- c("start_abs", "end_abs", "to_location", "label_x", "grob_x")
    one_g <- one_g |> dplyr::mutate_at(rev_vars, function(x) 120L - x)
  ...
  p <- pitch +
    ...
    ggplot2::annotate(
      ...
      label = ko_ret_td$lab,
      # family = "Roboto Condensed",
      hjust = ko_ret_td$lab_hjust, vjust = 0.5,
      size = 2
    ) +
    ...
    lapply(one_g$drive_number, function(x){
      ggforce::geom_link(data = one_g |> dplyr::filter(drive_number == x), ggplot2::aes(
        x = start_abs,
        y = y,
        xend = end_abs,
        yend = y,
  ...
  if(type != "all"){
    p <- p +
      ggrepel::geom_text_repel(
        data = plays,
        ggplot2::aes(x = yard_abs, y = y, label = play_count, color = count_color, bg.color = count_bg),
        size = 2, box.padding = 0, nudge_y = -1.75, direction = "x", bg.r = 0.10
      )
  }
  ...
  p <- p + nflplotR::geom_from_path(
    ...
    ggplot2::aes(x = grob_x, y = y, path = icon), width = 0.02
  ) +
  ...
  if (type == "all"){
    p <- p +
      ggplot2::geom_label(data = one_g |> dplyr::filter(posteam == home_team), ggplot2::aes(
        x = label_x,
        y = y,
        hjust = 1,
        label = label,
        color = team_color,
        alpha = 0.9,
      ), size = 2, nudge_x = -1, label.padding = unit(0.10, "lines")
      ) +
      ggplot2::geom_label(data = one_g |> dplyr::filter(posteam == away_team), ggplot2::aes(
        x = label_x,
        y = y,
        hjust = 0,
        label = label,
